...
- `src/bridge.py` — Telegram polling entrypoint. Handlers for `/start`, `/new`, `/model`, `/status`, `/ping`, `/stop`, `/help`, plus a text/media handler. Per-chat active task is stored in `_active_tasks`; a new message arriving while a task runs is **queued** in `_pending` (one slot — newer messages replace) and ack'd "📥 Queued". `/stop` cancels the active task and drops the queue. Per-chat liveness (started_at / last_chunk_at / bytes_streamed) in `_chat_liveness` drives `/ping` and the 5-min heartbeat. The Application is built with `concurrent_updates=True` so a queued message's `on_message` can run while the prior task is still streaming.
...
- **Queue (not steering)**: a new message during a running task is **queued** (one slot per chat, newer replaces) and ack'd, instead of cancelling. `/stop` is the explicit cancel. `/new` cancels + drops queue + resets the Claude session. The previous "new message cancels" steering behavior was removed because it lost in-progress work whenever the user typed a follow-up while away.
